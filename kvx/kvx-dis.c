...
#define KV3_OPC_COUNT RZ_ARRAY_SIZE(kv3_opc)
...
int kvx_instr_print(insn_t *insn, ut64 offset, char *buf, size_t len) {
	...
	while (n < (len - 1) && *fmt != '\0') {
		if (fmt[0] == '%' && fmt[1] == 's') {
			...
			if (i < RZ_ARRAY_SIZE(insn->opc->decode) && insn->opc->decode[i]) {
				insn->opc->decode[i](&opr, insn->value);
				if (opr.type == KVX_OPER_TYPE_IMM) {
					w = snprintf(buf + n, len - n, "0x%llx", opr.imm);
				} else if (opr.type == KVX_OPER_TYPE_OFF) {
					w = snprintf(buf + n, len - n, "0x%llx", opr.imm + offset);
...
static int disassemble_bundle(bundle_t *bundle, const ut32 *words, int count) {
	...
	for (i = 0; i < count; i++) {
		...
		case STEERING_BCU:
			...
			} else if (i == 1 && kvx_is_tca_opcode(word) && have_bcu) {
				/* special case: both BCU and TCA in the same bundle */
				issue = BUNDLE_ISSUE_TCA;
			} else {
		...
		case STEERING_ALU:
			if (!used[BUNDLE_ISSUE_ALU0]) {
				...
				rz_warn_if_reached(); /* too many ALUs */
				goto error;
			}
			break;
		case STEERING_MAU:
		...
		if (!used[issue]) {
			used[issue] = true;
			insn->len = 0;
		} else if (!have_ext && used[issue]) {
			rz_warn_if_reached(); /* issue already used */
			goto error;
		} else if (have_ext && !used[issue]) {
			rz_warn_if_reached(); /* missing issue */
			goto error;
		}
		if (insn->len == KVX_MAX_SYLLABLES) {
			rz_warn_if_reached(); /* too many syllables */
			goto error;
		}
...
static int read_bundle(ut32 *words, const ut8 *buf, int len) {
	...
	if (count == KVX_MAX_BUNDLE_WORDS && kvx_has_parallel_bit(word)) {
		rz_warn_if_reached(); /* this is wrong */
	}
